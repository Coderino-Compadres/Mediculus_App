...
As of 2026-08-26 both Azure databases are still at `0001` (faked); `0002`–`0007` have only been applied locally and need `migrate` on Azure. Three of them are written as `SeparateDatabaseAndState` with `ADD COLUMN IF NOT EXISTS`, because `database_setup.sql` declares the same columns and the documented setup order runs that script first: `0004_user_consents` adds the two RODO consent timestamps to `"user"`, and `0005_diary_entry_fields` adds the columns the "Dodaj wpis" form needs — `tension_level`, `emotion_note`, `thought`, `risky_behavior_note` on `diary`, plus `shame_scale`/`calm_scale` on `mood_scale`. `0005` targets medical_db only, so its `RunSQL` carries `hints={'target_db': 'medical'}`. `0006_drop_overall_feeling` then removes `diary.overall_feeling`: the "jakość samopoczucia" slider that would have filled it was cut from the entry form and `current_mood` already records how the patient says they feel, so it was a duplicate with no source. It is the first destructive migration — the text `mock_data.sql` used to seed there is not recoverable by reversing it. `0007_parent_child_accepted_at` adds `parent_child.accepted_at` (user_db, so `hints={'target_db': 'default'}`): NULL is a guardian invitation nobody has answered, set is the moment it was accepted — see the guardian-invitation notes below for why a refusal deletes the row instead of filling a third column. The ERD (`ERD/DIAGRAM.drawio`) was brought back in step with `0006` and now lists `diary` and `mood_scale` in the same order as `database_setup.sql`; **it does not yet show `parent_child.accepted_at`**, and neither do its `.png`/`.pdf` exports — all three are edited/generated by hand (there is no drawio CLI on the dev machine), so bring them in step after any schema change, this one included.
...
**Authentication: DRF endpoints under `/api/auth/`, session cookie, custom user table.** `core/urls.py` (mounted at `api/` by `config/urls.py`) exposes `csrf/`, `register/`, `login/`, `logout/`, `me/`, `guardian/`, plus `/api/guardian/invitations/`. The pieces worth knowing before touching them:
...
- **The guardian answers on `/modules`**, under the greeting, via `components/GuardianInvitations.tsx` — the first screen every account lands on after login, because a child is blocked until it is answered and a card behind a menu would be one nobody finds. Rendered only when `role === 'rodzic'`, so no other account even fires the request. A failed load says so rather than drawing nothing: silence would read as "nobody asked" while a child waits on the other side of it. The card carries the child's name and address and nothing clinical — accepting is a decision about a person, not a look at their diary.
- **Still missing:** a guardian cannot see or undo an *accepted* link (that is the parent panel's job, and `parent_child` is still the only table involved), and nothing notifies them out of band — an invitation is only visible to a guardian who logs in.
- **`LANGUAGE_CODE = 'pl'`**, so Django's own messages (password validators, DRF's "not authenticated") reach the Polish UI in Polish.
- **Rate limiting** on `login/`/`register/` is `10/min` per IP (and `10/min` per account on `auth/guardian/`, via `GuardianLinkThrottle`, a `UserRateThrottle` — `AnonRateThrottle` exempts requests carrying a session, so the per-IP cap would have counted nothing there), counted in the default local-memory cache — i.e. per gunicorn worker. A shared cache backend is what would make it a real limit.
