...
- **Three of the ten emotions have no scale column.** `mood_scale` covers seven, `diary.stress_level` supplies 'Stres', and 'Wstyd'/'Spokój' can only arrive as free text in `diary.current_strongest_emotion` — so a day can legitimately have a named emotion and no intensity, which the chart draws as a fixed-height bar in that emotion's colour.
