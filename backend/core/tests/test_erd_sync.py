...
#: diagram or this list catches up.
#:
#: **It is empty, which is the goal.** It stayed non-empty for four migrations
#: because the diagram is edited by hand (there is no drawio CLI on the dev
#: machine) and the backlog was easier to record than to draw; the specialist
#: panel's own three schema changes were the point at which drawing the backlog
#: was less work than explaining it. Keep it empty: if a change genuinely cannot
#: be drawn now, add the entry *with the reason*, the way those four were.
#:
#: What this does **not** check is the `.png`/`.pdf` exports, which nothing
#: checks and which are regenerated by hand from the `.drawio`. They are behind
#: this file — since 0004 for the consent columns, and now for everything the
#: specialist panel added.
KNOWN_ERD_GAPS = {}
...
class ErdDriftTests(SimpleTestCase):
    ...
    def test_diet_meal_matches(self):
        self.assert_table_matches(models.DietMeal)
    ...
    def test_administrator_matches(self):
        self.assert_table_matches(models.Administrator)

    def test_admin_audit_log_matches(self):
        self.assert_table_matches(models.AdminAuditLog)
